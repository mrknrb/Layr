...
export class ElementResizer {
	...
	constructor(element: ElementBaseClass) {
		...
		this.resizeActivate(this.elementObject.elementSettings.resizeType)
	}
	...
	public adjustSize() {
		...
		if (this.ActiveResizeType == ResizeTypes.autoXY) {
			...
			this.elementObject.nodeDiv.mainElement.element.style.width = (this.elementObject.element.scrollWidth) + "px"
